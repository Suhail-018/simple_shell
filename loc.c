...
char *get_location(char *command) {
    ...
    while (path_token != NULL) {
        ...
        command_path = malloc(command_length + token_length + 2); // Add missing semicolon.
        ...
        if (stat(command_path, &buffer) == 0) {
            /* Return value of 0 means success, implying that the command_path is valid */
            /* Free up allocated memory before returning your command_path */
            free(path_copy);
            return command_path;
        } else {
            free(command_path);
